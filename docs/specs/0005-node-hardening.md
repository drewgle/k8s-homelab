...
- **HARD-03** UFW default policy is deny incoming / allow outgoing / deny
  ...
  111/tcp+udp (rpcbind). Ceph ports (3300, 6789, 6800-7300/tcp) are allowed
  only from `ceph_public_network`. UFW is the *single* managed firewall:
  ...
  double-filter.
- **HARD-04** fail2ban jails cover sshd and the Proxmox web UI (custom
...
- **HARD-05** Kernel hardening sysctls are applied via
  ...
  links/fifos/regular). `net.ipv4.ip_forward=1` MUST stay enabled — Proxmox
...
- **HARD-07** auditd watches PVE config, SSH config, account files, time
  changes, and sessions; the ruleset is immutable (`-e 2`), so audit-rule
  changes REQUIRE a reboot to take effect.
- **HARD-08** Unattended security upgrades are enabled
  (`APT::Periodic::Unattended-Upgrade "1"` + distro-security origin).
- **HARD-09** Time sync uses chrony with the pools from the `ntp_servers`
  var; serving time to other hosts is disabled (allow 127.0.0.1, deny all).
